...
import { ViseRouter } from '@tencent/vise';
...
export function createApp(routerBase = '/') {
  ...
  if (IS_SSR) {
    // server端 renderContext.extra 中携带有routerBase
    finalBase = routerBase;
  } else {
    ...
    if (typeof ROUTE_BASE === 'string') {
      ...
      // 由于 vise 依赖 zx => fileURLToPath, 在__vite-browser-external 不存在，因此暂时没有用 @tencent/vise export 的 matchAppForUrl 来处理
